            ...
            <div class="row g-0">
                <div class="col d-flex flex-column">
                    <form id="insert-form" method="post" autocomplete="off" enctype="multipart/form-data">
                        <input type="hidden" id="inPer" name="inPer"
                        ...
                        <div class="card-body">
                            ...
                            <div class="row align-items-center">
                                ...
                                <input type='hidden' id="inPersonnelLogo64" name="inPersonnelLogo64"
                                    style="display:none;"
                                    value="<?php echo (!empty($row["profile_image"])) ? $row["profile_image"] : ""; ?>" />
                                <div class="col-auto">
                                    <span class="avatar avatar-xl" id="logo-image"
                                        style="background-image: url('<?php echo (!empty($row["profile_image"])) ? $row["profile_image"] : ""; ?>');">
                                    </span>
                                </div>
                                <div class="col-auto">
                            ...
                            <div class="row g-3">
                                <div class="col-md">
                                    <?php _label("Id Card", "required"); ?>
                                    <input type="text" class="form-control" id="inIdCard" name="inIdCard"
                                        value="<?php echo (!empty($row["idcard"])) ? $row["idcard"] : ""; ?>" />
                                </div>
                                <div class="col-md">
                                    <?php _label("Email", "required"); ?>
                                    <input type="text" class="form-control" id="inMail" name="inMail"
                                        value="<?php echo (!empty($row["mail"])) ? $row["mail"] : ""; ?>" required/>
                                </div>
                                <div class="col-md">
...
<script>
    ...
    $("#insert-form").submit(function (e) {
        ...
        $.ajax({
            type: "POST",
            url: " <?= site_url("PersonnelController/update_personnel"); ?>",
            data: $(this).serialize(),
        }).done(function (data) {
            alert("บันทึกสำเร็จ");
            location.href = "<?= site_url("personnel");?>";
        });
    });
</script>